...
namespace ICEEX.ApiTest.HttpTools
{
    public class HttpHelp
    {
        ...
        public T HttpPost<T>(string url, string apikey, string input, string sign, Encoding encoding = null)
        {
            ...
            request.Headers["X-ICEEX-APIKEY"] = apikey;
            request.Headers["X-ICEEX-SIGNATURE"] = sign;
            request.Headers["X-ICEEX-INPUT"] = input;
            var buffer = encoding.GetBytes("");
        ...
        public async Task<T> PostAsync<T>(string url,string apikey,string input,string sign)
        {
            ...
            using (HttpContent httpContent = new StreamContent(new MemoryStream(byteArray)))
            {
                ...
                httpContent.Headers.Add("X-ICEEX-APIKEY", apikey);
                httpContent.Headers.Add("X-ICEEX-SIGNATURE", sign);
                httpContent.Headers.Add("X-ICEEX-INPUT", input);
